...
###### error in the system is low - for now not doing any correction ######
...
# make a vairable that identifies each crab
...
save_object(crab.data, "crab_data.rds")
...
crwOut <- crawlWrap(obsData = obsData,
                    ...
                    retryFits = retryFits,
                    fixPar = c(NA, NA))
                    # include the below if you want to error-correct the positions
                    #fixPar = c(1, 1, NA, NA),
...
save_object(convergence, "crab_ctcrw_convergence.rds")

# save the output
save_object(crwOut, "crabctcrw.rds")
...
# save the output
save_object(prepData, "crab_prep.rds")